...
def batchScriptKNU(remoteDir, index):
   os.system("cp /tmp/x509*`id -u` /d/scratch/geonmo/")
...
cd $PBS_O_WORKDIR
eval `scramv1 ru -sh`
cp /d/scratch/geonmo/x509*`id -u`* /tmp
       ...
       if wantBatch:
          ...
          if ( socket.gethostname().split('.')[2] == 'uos') :
            ...
            scriptFile.write( batchScriptUOS( self.remoteOutputDir_,
                                                value) )
	  elif ( socket.gethostname().split('.')[1] == 'knu') :
            print '@ KNU'
            scriptFile.write( batchScriptKNU( self.remoteOutputDir_,
                                                value) )
          else :
            print '@ local'
